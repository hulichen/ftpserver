...
static void minimize_privilege()  //提升权限
{
	//更改nobody进程的信息
	...
	//设置头结构
	cap_header.version = _LINUX_CAPABILITY_VERSION_2;//64位系统
...
static void privop_pasv_get_data_sock(session_t* sess)
{
	...
	int sock = tcp_client(20);   //这里正常情况下需要绑定20端口
	...
	priv_sock_send_fd(sess->parent_fd, sock);  //发送socket给ftp进程
...
static void privop_pasv_listen(session_t* sess)
{
	...
	//0代表生成默认端口号
	int sockfd = tcp_server(ip, 0); //tcp_server中包含创建监听套接字
	...
	sess->pasv_listen_fd = sockfd;  //保存创建的监听套接字
...
static void privop_pasv_accept(session_t* sess)
{
	...
	{
		priv_sock_send_result(sess->parent_fd, PRIV_SOCK_RESULT_BAD); //创建失败
		return;
	}

	priv_sock_send_result(sess->parent_fd, PRIV_SOCK_RESULT_OK);//创建成功
